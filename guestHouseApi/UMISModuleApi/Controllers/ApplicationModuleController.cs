...
namespace UMISModuleAPI.Controllers
{
    ...
    public class ApplicationModuleController : ControllerBase
    {
        ...
        public IActionResult getMenu(int roleId, int moduleId)
        {
            try
            {
                cmd = "select distinct applicationModuleID,menuId, applicationModuleTitle,menuTitle,moduleIcon,menuIcon, parentRoute, routeTitle,[read], [write], [delete], parentMenuId, menuSeq from view_user_role_details where roleID = " + roleId + " AND applicationModuleID = " + moduleId + " order by menuSeq Asc";
