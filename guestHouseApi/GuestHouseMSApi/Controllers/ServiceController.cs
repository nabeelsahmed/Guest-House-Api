...
namespace GuestHouseMSApi.Controllers
{
    ...
    public class ServiceController : ControllerBase
    {
        ...
        [HttpGet("getServices")]
        public IActionResult getServices(int branchID,int serviceTypeID,int roomBookingDetailID)
        {
            try
            {
                if (serviceTypeID == 0 && branchID !=0 && roomBookingDetailID !=0)
                {
                    cmd = @"select * from view_services where branch_id = "+branchID+" and roomBookingDetailID = "+roomBookingDetailID+"";
                }
                else if (serviceTypeID != 0 && branchID !=0 && roomBookingDetailID !=0)
                {
                    cmd = @"select * from view_services where branch_id = "+branchID+" and roomBookingDetailID = "+roomBookingDetailID+" and serviceTypeID = " + serviceTypeID + "";
                }
                else
                {
                    cmd = @"select * from view_services";
                }
